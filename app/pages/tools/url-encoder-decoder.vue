...
useSeoMeta({
  title: 'URL Encoder / Decoder - Playground Sunshine',
  description:
    'Encode or decode percent-encoded URLs directly in your browser with this free, privacy-friendly tool.',
})
...
const result = computed(() =>
  mode.value === 'encode' ? encodeUrl(input.value) : decodeUrl(input.value),
)
...
<template>
  <div class="max-w-3xl mx-auto px-4 py-10 space-y-6">
    <!-- Header -->
    <div>
      <h1 class="text-2xl font-bold text-gray-900">URL Encoder / Decoder</h1>
      <p class="mt-1 text-gray-500">
        Encode a URL with percent-encoding or decode it back to plain text.
      </p>
    </div>
    ...
    <div class="space-y-1">
      <label for="url-input" class="block text-sm font-medium text-gray-700">
        {{ mode === 'encode' ? 'Plain URL or Text' : 'Encoded URL' }}
      </label>
      <textarea
        id="url-input"
        v-model="input"
